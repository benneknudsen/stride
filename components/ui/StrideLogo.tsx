...
const FILLS: Record<Tone, [string, string, string, number, number]> = {
  ...
  // `duo` greys track the foreground token so the mark stays legible in both themes.
  duo: ["var(--color-fg)", "var(--color-fg)", "#C6F432", 0.55, 0.8],
  ...
  return (
    <svg width={size} height={(size * 84) / 96} viewBox="0 0 96 84" fill="none" aria-hidden="true">
      <g transform="translate(13,0) skewX(-13)">
        <rect x="8" y="44" width="16" height="26" rx="7" style={{ fill: s }} opacity={so} />
        <rect x="33" y="30" width="16" height="40" rx="7" style={{ fill: m }} opacity={mo} />
        <rect x="58" y="14" width="16" height="56" rx="7" style={{ fill: t }} />
      </g>
    </svg>
  );
}
...
export function StrideLogo({ tone = "duo", size = 24 }: { tone?: Tone; size?: number }) {
  const word = tone === "ink" ? "#0B0D11" : "var(--color-fg)";
  return (
